...
MainController::MainController(int argc, char* argv[])
  ...
  end = std::numeric_limits<uint16_t>::max(); // Funny bound, since we predict times in this format
                                              // really!
...
void MainController::run() {
  ...
  while (!pangolin::ShouldQuit() && !((!logReader->hasMore()) && quiet) && ros::ok() &&
    ...
    if (gui->followPose->Get()) {
      ...
      Eigen::Matrix4d m;
      m << x(0), x(1), x(2), -(x.dot(eye)), y(0), y(1), y(2), -(y.dot(eye)), z(0), z(1), z(2),
          -(z.dot(eye)), 0, 0, 0, 1;
    ...
    stre
        << (std::isnan(eFusion->getModelToModel().lastICPError)
                ? 0
                : eFusion->getModelToModel().lastICPError);
    ...
    if (gui->drawDefGraph->Get()) {
      ...
      for (size_t i = 0; i < graph.size(); i++) {
        pangolin::glDrawCross(
            graph.at(i)->position(0), graph.at(i)->position(1), graph.at(i)->position(2), 0.1);

        for (size_t j = 0; j < graph.at(i)->neighbours.size(); j++) {
          pangolin::glDrawLine(
              graph.at(i)->position(0),
